...
class ViewController: UIViewController, CLLocationManagerDelegate, MKMapViewDelegate {
    ...
    @IBAction func button_clicked(sender: AnyObject) {
        ...
        //create popup dialog
        var alert = UIAlertController(title: "Update Location", message: nil, preferredStyle: UIAlertControllerStyle.Alert)
        ...
        alert.addAction(UIAlertAction(title: "Update Location", style: .Default, handler: {(action: UIAlertAction!) in
            ...
            time = lastUpdated.substringWithRange(Range(start: advance(lastUpdated.startIndex, 11), end: advance(lastUpdated.startIndex, 16)))
            
            
            //parses the date
            if(Array(lastUpdated)[0] == "0" && lastUpdated.substringWithRange(Range(start: advance(lastUpdated.startIndex,11), end: advance(lastUpdated.startIndex, 13))).toInt() <= 12)
            {
            ...
            var bodyData:String
            bodyData = "Latitude=" + toString(self.latitude) + "&Longitude=" + toString(self.longitude) + "&Username=rockyfish&Address=" + self.address + "&Comments=" + self.comment + "&LastUpdated=" + lastUpdated + "&Number=MTAwMDAwMDEzMw==&Time=" + time;
            var url:NSURL = NSURL(string: "http://www.skyrealmstudio.com/cgi-bin/updatelocation.py")!
            ...
            self.mapView.addAnnotation(self.userLocationAnnotation)
                ...
                println("Could not update location")
            }
        }))
    ...
    func get_marker_script()
    {
        ...
        var url:NSURL = NSURL(string: "http://skyrealmstudio.com/cgi-bin/MarkerScript.py")!
        var request:NSMutableURLRequest = NSMutableURLRequest(URL: url)
        request.HTTPMethod = "POST"
        request.HTTPBody = bodyData.dataUsingEncoding(NSUTF8StringEncoding)
        NSURLConnection.sendAsynchronousRequest(request, queue: NSOperationQueue.mainQueue())
            {
                ...
                let jsonObject : AnyObject! = NSJSONSerialization.JSONObjectWithData(data, options: NSJSONReadingOptions.MutableContainers, error: nil)
                    if let statusesArray = jsonObject as? NSArray
                    {
                        ...
                        {
                            let user: String = toString(aStatus["Username"]!)
                            let lat: Double? = aStatus["Latitude"]?.doubleValue
                            let long: Double? = aStatus["Longitude"]?.doubleValue
                            let comm: String = toString(aStatus["Comment"]!)
                            let lastUpdated: String = toString(aStatus["LastUpdated"]!)
                            let description: String = lastUpdated.substringWithRange(Range(start: advance(lastUpdated.startIndex, 5), end: advance(lastUpdated.startIndex,16))) + lastUpdated.substringWithRange(Range(start: advance(lastUpdated.startIndex, 20), end: advance(lastUpdated.startIndex, 22))) + " - " + comm
                            
                            let annotation:CustomPointAnnotation = CustomPointAnnotation()
                            annotation.title = toString(user)
